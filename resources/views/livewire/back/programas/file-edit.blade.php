                ...
                <div class="card shadow-sm">
                    <div class="card-header">
                        ...
                        <div class="card-toolbar">
                            ...
                            <button type="button" wire:click='btndescargarFilePrograma({{ $programa->id }})'
                                class="btn btn-sm btn-icon  btn-outline btn-outline-dashed btn-outline-dark btn-active-light-dark me-2">
                                <i class="fas fa-download"></i>
                            </button>
                            <button type="button" wire:click="$emit('deleteFilePrograma', {{ $programa->id }})"
                    ...
                    <div class="card-body p-0 shadow">
                        <div class="mb-5">
                            <div class="text-center px-4 pt-5 pb-2">
                                ...
                                <p class="pt-2">{{
                                 str_replace('storage/programas/',"", $programa->archivo);}}</p>
                            </div>
                        </div>
                    </div>
                </div>
...
@push('js')
    <script>
         Livewire.on('deleteFilePrograma', dataID => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Livewire.emit('btnDeleteFilePrograma', dataID);
                    Swal.fire({
                        title: 'Eliminado',
                        confirmButtonColor: '#212529',
                        icon: 'success',
