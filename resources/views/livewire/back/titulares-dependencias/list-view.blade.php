...
<div>
    <div class="row mb-10 pt-6">
        ...
        <div class="col-sm-12 col-md-4">
            <div class="mb-5">
                <label for="search_secretario" class="form-label">Secretario</label>
                <input type="text" class="form-control" wire:model.defer="search_secretario" id="search_secretario"
                    placeholder="Secretario..." />
            </div>
        </div>
        <div class="col-sm-12 col-md-4">
            <div class="mb-5">
                <label for="search_secretaria" class="form-label">Secretaría</label>
                <select name="search_secretaria" class="form-select" wire:model.defer="search_secretaria">
                    <option value="" selected >Selecciona...</option>
                    @foreach ($secretarias as $s)
                        <option value="{{$s->id}}">{{$s->secretaria}}</option>
                    @endforeach
                </select>
        ...
        <div class="col-sm-12 col-md-4">
            <div class="mb-5">
                <label for="search_telefono" class="form-label">Telefóno</label>
                <input type="text" class="form-control" wire:model.defer="search_telefono" id="search_telefono"
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table wire:sortable="updateDependenciaOrder"  id="kt_datatable_horizontal_scroll" class="table table-striped table-row-bordered gy-5 gs-7">
                    <thead>
                    ...
                    <tbody>
                        @if (count($data) > 0)
                            @foreach ($data as $d)
                                <tr  wire:sortable.item="{{ $d->id }}" wire:key="dependecia-{{ $d->id }}" wire:sortable.handle>
                                    ...
                                    <td><img src="{{asset($d->img)}}" alt="{{$d->nombre}}" width="50"></td>
                                    <td>{{ $d->nombre }}</td>
                                    ...
                                    <td style="max-width: 160px;">
                                        ...
                                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-400 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                            ...
                                            <div class="menu-item px-3">
                                                <a href="{{ route('sistema.titulares.dependencia.show', $d->id) }}"
                                            ...
                                            <!--begin::Menu item-->
                                            <div class="menu-item px-3">
                                                <a href="{{ route('sistema.titulares.dependencia.edit', $d->id) }}"
                                                    class="menu-link px-3">
                                                    Editar
                                                </a>
                                            </div>
                                            ...
                                            <!--begin::Menu item-->
                                            <div class="menu-item px-3">
                                                <a href="#" wire:click="$emit('deleteTituDepend',{{$d->id}})" data-kt-subscriptions-table-filter="delete_row"
                                                    class="menu-link px-3">
                                                    Eliminar
                                                </a>
                                            </div>
                        ...
                        @else
                            <tr>
                                <td colspan="7" class="text-center">
                                    ...
                                    <br>
                                    <a href="{{ route('sistema.titulares.dependencia.create') }}" class="btn btn-dark">Registrar
                                        un Titular de
                                        Dependencia</a>
                                </td>
                            </tr>
...
<script>
   window.addEventListener('dropCreate', function(e) {
    ...
    Livewire.on('deleteTituDepend', dataID => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            Livewire.emit('deleteTitularDependencias', dataID);
            Swal.fire({
                title: 'Eliminado',
                confirmButtonColor: '#212529',
                icon: 'success',
