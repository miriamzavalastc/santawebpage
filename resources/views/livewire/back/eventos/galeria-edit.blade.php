                    ...
                    <div class="card shadow-sm">
                        <div class="card-header">
                            ...
                            <div class="card-toolbar">
                                <button type="button" wire:click='btndescargarEvento({{ $gal->id }})'
                                    class="btn btn-sm btn-icon  btn-outline btn-outline-dashed btn-outline-dark btn-active-light-dark me-2">
                                    <i class="fas fa-download"></i>
                                </button>
                                <button type="button" wire:click="$emit('deleteImageGaleriaEventos',{{ $gal->id }})"
...
@push('js')
    <script>
         Livewire.on('deleteImageGaleriaEventos', dataID => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Livewire.emit('btnDeleteImageGaleria', dataID);
                    Swal.fire({
                        title: 'Eliminado',
                        confirmButtonColor: '#212529',
                        icon: 'success',
