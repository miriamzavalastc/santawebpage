...
<div>
    <div class="row mb-10 pt-6">
        ...
        <div class="col-sm-12 col-md-4">
            <div class="mb-5">
                <label for="search_portada" class="form-label">En Portada?</label>
                <select class="form-select" id="search_portada" wire:model.defer="search_portada"
                    ...
                    <thead>
                        <tr class="fw-semibold fs-6 text-gray-800 border-bottom-2 border-gray-200 bg-gray-300 ">
                           <th>FOTO</th>
                            ...
                            <th>FECHA FINAL</th>
                            <th>CATEGORIA</th>
                            <th class="text-center">EN PORTADA</th>
                        ...
                        @if (count($data) > 0)
                            @foreach ($data as $d)
                                <tr >
                                    ...
                                    <td>{{ $d->nombre }}</td>
                                    <td>{{ \Carbon\Carbon::parse($d->fecha_inicio)->format('d/m/Y') }}</td>
                                    <td>{{ \Carbon\Carbon::parse($d->fecha_final)->format('d/m/Y') }}</td>
                                    ...
                                    <td style="max-width: 130px;">
                                        ...
                                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-400 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                            ...
                                            <div class="menu-item px-3">
                                                <a href="{{ route('sistema.eventos.edit', $d->id) }}"
                                            ...
                                            <div class="menu-item px-3">
                                                <a href="#"
                                                    wire:click="$emit('deleteEventosShow',{{ $d->id }})"
                        ...
                        @else
                            <tr>
                                <td colspan="8" class="text-center">
                                    ...
                                    <br>
                                    <a href="{{ route('sistema.eventos.create') }}"
                                        class="btn btn-dark">Registrar
                                        un
                                        Evento</a>
                                </td>
                            </tr>
...
@push('js')
    ...
    <script>
        ...
        Livewire.on('deleteEventosShow', dataID => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Livewire.emit('deleteEvento', dataID);
                    Swal.fire({
                        title: 'Eliminado',
                        confirmButtonColor: '#212529',
                        icon: 'success',
