                ...
                <table   id="kt_datatable_horizontal_scroll" class="table table-striped table-row-bordered gy-5 gs-7">
                    ...
                    <tbody>
                        @if (count($data) > 0)
                            @foreach ($data as $d)
                                <tr >
                                    <td class="symbol symbol-30px"> <img src="{{ Avatar::create($d->name)->toBase64() }}" alt="{{  $d->name }}" /></td>
                                    <td>{{ $d->name }}</td>
                                    <td>{{ $d->email }}</td>
                                    <td>
                                        @foreach($d->getRoleNames() as $r)
                                            ...
                                            @if($r != 'SystemUser')
                                                <span class="badge @if($r === 'Administrador') bg-success @else bg-secondary @endif">{{$r}}</span>
                                            @endif
                                    ...
                                    <td style="max-width: 130px;">
                                        ...
                                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-400 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                            ...
                                            <div class="menu-item px-3">
                                                <a href="{{ route('sistema.users.edit', $d->id) }}"
                                            ...
                                            <!--begin::Menu item-->
                                            <div class="menu-item px-3">
                                                <a href="#" wire:click="$emit('btnDeleteUsers',{{$d->id}})" data-kt-subscriptions-table-filter="delete_row"
                                                    class="menu-link px-3">
                                                    Eliminar
                                                </a>
                                            </div>
                        ...
                        @else
                            <tr>
                                <td colspan="6" class="text-center">
                                    ...
                                    <br>
                                    <a href="{{ route('sistema.users.create') }}" class="btn btn-dark">Registrar
                                        un Usuario</a>
                                </td>
                            </tr>
...
<script>
    ...
    Livewire.on('btnDeleteUsers', dataID => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            Livewire.emit('deleteUser', dataID);
            Swal.fire({
                title: 'Eliminado',
                confirmButtonColor: '#212529',
                icon: 'success',
